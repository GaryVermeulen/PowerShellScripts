...
<# 
	...
	.NOTES 
        ...
        v2.0; GV
        -Changed name
        -Added[ing] more scanning checks for non-Windows devices in an attempt to identify
        ...
        -Added HTTP(S) check
        -Drop through to arp if nbtstat returns 00-00-00-00-00-00 

        v1.4; GV
        -Check arp if nbtstat returns null
        -Scan for serial number & BIOS ver
...
#######################					
# This code was in the "begin" section, but some devices were caught for no known reason--yet another work-around
#
...
#######################
					...
					if(Test-Connection -ComputerName $IP -Quiet)
					{
                        ...
                        catch
                        {
                         ...
                         try
                        {
                            $webr = Invoke-Webrequest -URI "https://$IP"
                            $HTTPSCode = $webr.StatusCode
                        }
                        catch
                        {
                            $HTTPSCode = $Error[0]
	...
	End{
        Add-Content -Path $myLogFile -Value "*Finis*"
    }
}
